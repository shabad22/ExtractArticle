...
$input = new SplFileObject('./output-pa/align.data.tagged');
$output = new SplFileObject('./output-pa/align.data.tagged.cleaned', "a");
...
while(!$input->eof()) {
	...
	foreach($in_line as $word) {
		if(in_array($word[1], ['N_NN', 'N_NNP'])) {
		//if(in_array($word[1], ['N_NN'])) {
			$out_line .= "$word[0] ";
		}
	}
	/**
